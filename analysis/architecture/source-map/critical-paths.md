...
| Inbox 如何区分 followup/steer/inject？ | [`core/agent-loop/src/inbox.ts`](../../../packages/core/agent-loop/src/inbox.ts) | `ReactLoopAgent.send()` 分类与 wake latch |
...
| Chunk 如何成为 message？ | [`core/agent-loop/src/agent.ts`](../../../packages/core/agent-loop/src/agent.ts) `step()` | `BlockAssembler` + 紧凑流 → assistant/message 或 assistant/attempt；实时帧走 agent/assistant-stream |
...
| Write-behind 谁协调？ | [`session-persistence-jsonl/src/storage.ts`](../../../packages/session/session-persistence-jsonl/src/storage.ts) | 单写者句柄 → live 事件批处理 → append/flush/close |
...
| Web API 如何进入浏览器？ | [`api/gateway/src/index.ts`](../../../packages/api/gateway/src/index.ts) | Connection → Remote controller → Client store |
...
| 问题 | 入口 | 后续路径 |
|---|---|---|
| 桌面如何启动后端？ | [`apps/desktop/src/host-process.ts`](../../../apps/desktop/src/host-process.ts) | 内置 Node → Desktop Host → 字节管道 |
| UI 如何同步历史与实时流？ | [`api/session-controller/src/history.ts`](../../../packages/api/session-controller/src/history.ts) | snapshot + cursor → durable events + transient frames |
| 旧 Session 如何打开？ | [`session-persistence-jsonl/src/generation.ts`](../../../packages/session/session-persistence-jsonl/src/generation.ts) | 最高代际 → 静态迁移链 → 只读结果或排他发布 |
